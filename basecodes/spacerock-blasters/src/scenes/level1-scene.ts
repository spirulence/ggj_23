...
export default class LevelOne extends Phaser.Scene {
  ...
  create() {
    ...
    this.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.ONE).onDown =
      launcher(100, 580, pointer, bullets);
    this.launchers.push(this.add.sprite(100, 580, "launcher"));

    this.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.TWO).onDown =
      launcher(400, 580, pointer, bullets);
    this.launchers.push(this.add.sprite(400, 580, "launcher"));

    this.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.THREE).onDown =
      launcher(700, 580, pointer, bullets);
  ...
  update(time: number, delta: number): void {
    if (Math.random() < 1.0 / 150.0) {
      ...
      const rock: Phaser.Physics.Arcade.Image = this.rocks.create(
        Math.random() * 600.0 + 100.0,
        -100
      );
      rock.scale = 0.2;
    ...
    this.launchers.forEach((launcher) => {
      const directionVector = launcher
        .getCenter()
        .subtract(this.input.activePointer.position);
